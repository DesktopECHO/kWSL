...
ECHO [kWSL Installer 20201123-10]
ECHO:
...
ECHO [%TIME:~0,8%] apt-fast Downloader (~0m45s)
%GO% "DEBIAN_FRONTEND=noninteractive apt-get -y install /tmp/kWSL/deb/aria2_1.35.0-1build1_amd64.deb /tmp/kWSL/deb/libaria2-0_1.35.0-1build1_amd64.deb /tmp/kWSL/deb/libssh2-1_1.8.0-2.1build1_amd64.deb /tmp/kWSL/deb/libc-ares2_1.15.0-1build1_amd64.deb --no-install-recommends" > ".\logs\%TIME:~0,2%%TIME:~3,2%%TIME:~6,2% apt-fast Downloader .log" 2>&1
...
REM %GO% "sed -i 's#Exec=gksu /usr/bin/x-terminal-emulator#Exec=kdesu -n --noignorebutton -d -- bash -c +source /etc/profile.d/kWSL.sh ; konsole+#g' /usr/share/applications/gksu.desktop ; sed -i 's#+#\"#g' /usr/share/applications/gksu.desktop"
REM %GO% "mv /usr/bin/pkexec /usr/bin/pkexec.kWSL ; echo gksudo -k -S -g \$1 > /usr/bin/pkexec ; chmod 755 /usr/bin/pkexec"
