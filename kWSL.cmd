...
START /MIN /WAIT "Acquire KDE Neon Keys" %GO% "apt-key adv --keyserver keyserver.ubuntu.com --recv-keys E6D4736255751E5D B7B9C16F2667CA5C ; mv /etc/apt/trusted.gpg /etc/apt/trusted.gpg.d/kWSL.gpg"
...
%GO% "cp -Rp /tmp/kWSL/dist/* / ; cp -Rp /tmp/kWSL/dist/etc/skel/.cache /root ; cp -Rp /tmp/kWSL/dist/etc/skel/.config /root ; cp -Rp /tmp/kWSL/dist/etc/skel/.local /root ; cp /tmp/kWSL/dist/etc/init.d/xrdp /etc/init.d/ ; chmod +x /etc/init.d/xrdp ; update-rc.d -f xrdp defaults"
START /MIN /WAIT "Updates for WSL1 Compatibility" "%DISTROFULL%\LxRunOffline.exe" "r" "-n" "%DISTRO%" "-c" "dpkg -i /tmp/kWSL/deb/libkf5activitiesstats*.deb /tmp/kWSL/deb/kactivitymanagerd*.deb /tmp/kWSL/deb/kinfocenter*.deb"
%GO% "apt-mark hold kactivitymanagerd libkf5activitiesstats1 kinfocenter" > NUL
