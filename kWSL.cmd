...
SET DISTRO=kWSL& SET /p DISTRO=Keep the name simple, no space or underscore characters [kWSL]: 
...
ECHO:
ECHO Update SSL Libraries...
CD "%DISTROFULL%"
ECHO kWSL INPUT SPECIFICATIONS // Path: %DISTROFULL% // Distro: %DISTRO% // RDP Port: %RDPPRT% // SSH Port: %SSHPRT% > Step0_Inputs.log
...
%GO% "cd /tmp ; apt-get -y install ./devuan-keyring_2017.10.03_all.deb ./ca-certificates_20200601~deb9u1_all.deb ./openssl_1.1.0l-1~deb9u1_amd64.deb ./libssl1.1_1.1.0l-1~deb9u1_amd64.deb" > "Step1_SSL.log"
...
%GO% "apt-key adv --recv-keys --keyserver keyserver.ubuntu.com 2667CA5C ; apt-get update ; apt-get -y install seamonkey-mozilla-build" > Step10_Mozilla.log
%GO% "update-alternatives --install /usr/bin/www-browser www-browser /usr/bin/seamonkey 100 ; update-alternatives --install /usr/bin/gnome-www-browser gnome-www-browser /usr/bin/seamonkey 100 ; update-alternatives --install /usr/bin/x-www-browser x-www-browser /usr/bin/seamonkey 100" > Step11_Defaults.log
ECHO:
ECHO Install VLC...
%GO% "DEBIAN_FRONTEND=noninteractive apt-get -y install vlc" > Step12_MediaPlayer.log
...
ECHO:
ECHO Cleaning up...
%GO% "apt-get -qq purge iptables exim4-base cryptsetup cryptsetup-bin cryptsetup-initramfs cryptsetup-run irqbalance multipath-tools apparmor snapd squashfs-tools plymouth  open-vm-tools cloud-init isc-dhcp-* gnustep* lvm2* mdadm apport open-iscsi powermgmt-base popularity-contest fwupd libfwupd2 ; apt-get -qq autoremove ; apt-get -qq clean" > Step13_Cleanup.log
...
%GO% "ssh-keygen -A ; strip --remove-section=.note.ABI-tag /usr/lib/x86_64-linux-gnu/libQt5Core.so.5" > NUL
